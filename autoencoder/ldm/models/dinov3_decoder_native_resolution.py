...
class DinoDecoder(pl.LightningModule):
    ...
    def forward(self, x_list):
        """Encode then decode a list of inputs."""
        z_list = [self.encode(x) for x in x_list]
        return [self.decode(z) for z in z_list]

    def get_input_list(self, batch):
        """Prepare input batch and normalize for DINO."""
        x_list = [img.unsqueeze(0) if img.ndim == 3 else img for img in batch]
        x_dino_list = []
        for x in x_list:
            x_dino = (x + 1.0) / 2.0
            mean = torch.tensor([0.485, 0.456, 0.406], device=x.device).view(1, 3, 1, 1)
            std = torch.tensor([0.229, 0.224, 0.225], device=x.device).view(1, 3, 1, 1)
            x_dino_list.append((x_dino - mean) / std)
        return x_list, x_dino_list

    def training_step(self, batch, batch_idx):
        x_list, x_dino_list = self.get_input_list(batch)
        recon_list = self(x_dino_list)

        ae_opt, disc_opt = self.optimizers()

        # Autoencoder update
        aeloss, log_ae = self.loss(x_list, recon_list, 0, self.global_step,
        ...
        discloss, log_disc = self.loss(x_list, recon_list, 1, self.global_step,
                                       last_layer=self.get_last_layer(), split="train")
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        ...
        if not only_inputs:
            recon_list = self(x_dino_list)
            for i, rec in enumerate(recon_list):
                if rec.shape[1] > 3:
                    x_list[i] = self.to_rgb(x_list[i])
                    recon_list[i] = self.to_rgb(rec)
            log["reconstructions"] = recon_list
        log["inputs"] = x_list
