...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        print("!!!!")
        print("This Encoder is customized by Haolin Wang.")
        print("!!!!")
        ...
        self.pos_embedding = nn.Parameter(torch.empty(1, seq_length, hidden_dim).normal_(std=0.02))  # from BERT
        self.dropout = nn.Dropout(dropout)
        self.base_seq_length = seq_length
    ...
    #     input = input + self.pos_embedding
    #     return self.ln(self.layers(self.dropout(input)))
    def interpolate_pos_embedding_2d(self, x: torch.Tensor):
        B, L, D = x.shape

        # cls_token 的 pos_embedding
        cls_pos_emb = self.pos_embedding[:, :1, :].expand(B, -1, -1).contiguous()
        
        # patch tokens
        patch_tokens = x[:, 1:, :]
        N = patch_tokens.shape[1]
        H0 = W0 = int(self.base_seq_length ** 0.5)
        H = W = int(N ** 0.5)

        # patch 的 pos_embedding 插值
        pos_emb_patch = self.pos_embedding[:, 1:, :].reshape(1, H0, W0, D).permute(0, 3, 1, 2).contiguous()
        pos_emb_patch = F.interpolate(pos_emb_patch, size=(H, W), mode='bicubic', align_corners=False)
        pos_emb_patch = pos_emb_patch.permute(0, 2, 3, 1).reshape(1, N, D).contiguous()
        pos_emb_patch = pos_emb_patch.expand(B, -1, -1).contiguous()

        # 拼回 cls_token
        pos_emb = torch.cat([cls_pos_emb, pos_emb_patch], dim=1).contiguous()
        return pos_emb

    def forward(self, input: torch.Tensor):
        """
        input: (B, L, D), L 可以根据 patch 数量随输入图像分辨率变化
        """
        torch._assert(input.dim() == 3,
                      f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        pos_emb = self.interpolate_pos_embedding_2d(input)
        x = input + pos_emb
        x = x.contiguous()
        x = self.dropout(x)
        x = self.layers(x)
