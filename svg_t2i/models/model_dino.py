...
# 尝试导入 FlashAttention 模块
try:
    from flash_attn import flash_attn_varlen_func
    from flash_attn.bert_padding import index_first_axis, pad_input, unpad_input  # noqa
    _HAS_FLASH_ATTN = True
    print("FlashAttention imported successfully. Using optimized path.")
except ImportError:
    # 如果导入失败，设置标志位为 False，并在 JointAttention 中使用 PyTorch SDPA
    flash_attn_varlen_func = None
    index_first_axis = None
    pad_input = None
    unpad_input = None
    _HAS_FLASH_ATTN = False
    print("FlashAttention not found. Falling back to standard PyTorch attention.")
...
class JointAttention(nn.Module):
    ...
    def forward(
        ...
        # ====== Key logic: determine whether to use FlashAttention ======
        global _HAS_FLASH_ATTN
        if _HAS_FLASH_ATTN and dtype in [torch.float16, torch.bfloat16]:
            # begin var_len flash attn (only executed when FlashAttention is available and dtype is FP16/BF16)
            (
                query_states,
                key_states,
                value_states,
            ...
            # Standard PyTorch attention path (GQA logic)
...
class NextDiT(nn.Module):
    ...
    def forward_with_cfg(
        ...
        if mode == "standard":
            # Take the first half of the batch. Assumes input is [cond, uncond].
            half = x[: len(x) // 2]

            # Duplicate the half to form [cond, uncond] → [half, half]
            combined = torch.cat([half, half], dim=0)

            # 1. Run the model to get output predictions
            # model_out shape is typically [2 * batch, C, H, W]
            model_out = self.forward(combined, t, cap_feats, cap_mask)

            # 2. Extract the predicted noise (eps)
            # Only keep the first `self.in_channels` channels
            eps = model_out[:, :self.in_channels]

            # 3. Split into conditional and unconditional predictions
            # Assumes eps = [cond_eps, uncond_eps]
            cond_eps, uncond_eps = torch.split(eps, len(eps) // 2, dim=0)

            # Optional scaling on cond (used for ablations/testing)
            ratio = 1
            cond_eps = cond_eps * ratio

            # 4. Standard CFG formula:
            #     eps = uncond + s * (cond - uncond)
            half_eps = uncond_eps + cfg_scale * (cond_eps - uncond_eps)

            # 5. Restore the expected batch dimension
            # Many pipelines expect output batch size = input batch size
            ...
            if t[0] < cfg_trunc:
                ...
                if float(renorm_cfg) > 0.0: 
                    ...
                    scale = torch.minimum(torch.ones_like(new_pos_norm), max_new_norm/(new_pos_norm + 1e-12))
                    half_eps = half_eps * scale
            else:
                combined = half
...
def NextDiT_08B_GQA_patch1_Adaln_Refiner(**kwargs):
    return NextDiT(
        patch_size=1,
        dim=1152,
        n_layers=28,
        ...
        n_kv_heads=8,
        axes_dims=[24, 24, 24],
        axes_lens=[600, 512, 512],
        **kwargs
    )


def NextDiT_2B_GQA_patch1_Adaln_Refiner(**kwargs):
    return NextDiT(
        patch_size=1,
        dim=2304,
        n_layers=26,
...
def NextDiT_2B_GQA_patch1_Adaln_Refiner_Residual(**kwargs):
    return NextDiT(
        patch_size=1,
        dim=2304,
        n_layers=26,
