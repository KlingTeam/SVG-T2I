...
class Decoder(nn.Module):
    ...
    def forward(self, z: Tensor) -> Tensor:
        ...
        # z to block_in
        if self.block_out_channels[0] % self.z_channels == 0:
            # print(self.block_out_channels[0] % self.z_channels)
            repeats = self.block_out_channels[0] // (self.z_channels)
            h = self.conv_in(z) + z.repeat_interleave(repeats=repeats, dim=1)
        else:
            # print("here")
            ### hard code
            repeats = self.block_out_channels[0] // 8
            h = self.conv_in(z) + z[:,-8:,...].repeat_interleave(repeats=repeats, dim=1)
