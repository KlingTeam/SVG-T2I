...
class LPIPSWithDiscriminatorDecoder(nn.Module):
    def __init__(self, disc_start, distill_start, logvar_init=0.0, pixelloss_weight=1.0,
        ...
        self.distill_iter_start = distill_start
    ...
    def forward(self, inputs, reconstructions, optimizer_idx,
        ...
        if optimizer_idx == 0:
            ...
            if self.distill_factor > 0.0 and student_features is not None and teacher_features is not None:
                distill_factor = adopt_weight(self.distill_factor, global_step, threshold=self.distill_iter_start)
                ...
                last_layer_student_feat = student_features[-1]
                last_layer_teacher_feat = teacher_features[-1]

                similarity = F.cosine_similarity(last_layer_student_feat, last_layer_teacher_feat.detach(), dim=-1).mean(dim=1)  
                ...
                distillation_loss = torch.mean(distance)

                if split == 'train':
                    distill_weight = self.calculate_adaptive_weight_distill(nll_loss, distillation_loss, last_layer=enc_last_layer)
                else:
                    distill_weight = 1.0
                loss = weighted_nll_loss  +  d_weight * disc_factor * g_loss + distill_weight * distill_factor * distillation_loss
            else:
            ...
            log = {"{}/total_loss".format(split): loss.clone().detach().mean(), "{}/logvar".format(split): self.logvar.detach(),
                   ...
                   "{}/distill_factor".format(split): torch.tensor(distill_factor),
